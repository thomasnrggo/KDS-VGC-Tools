 ...
 */
export function PokemonReportRow({
  pokemon,
  note,
  onNoteChange,
}: PokemonReportRowProps) {
  ...
  return (
    <div className="grid grid-cols-1 gap-3 border-b border-mauve-300/60 p-4 last:border-b-0 md:grid-cols-[100px_150px_170px_1fr] md:items-start md:gap-4">
      ...
      <div className="flex flex-col gap-1">
        ...
        {breakdown ? (
          <table className="w-full text-[11px] leading-tight text-mauve-700">
            <thead>
      ...
      <div className="flex flex-col gap-1">
        ...
        {pokemon.moves && pokemon.moves.length > 0 ? (
          ...
          <p className="text-xs text-mauve-400">
            No moves parsed for this Pokémon.
          </p>
        )}
      </div>
      ...
      <div className="flex min-w-0 flex-col gap-1">
        ...
        <textarea
          ...
          className="min-h-24 max-h-40 w-full resize-none overflow-y-auto rounded-lg bg-white/60 p-2 text-sm text-mauve-800 focus:outline-none focus:ring-2 focus:ring-mauve-400"
        />
      </div>
    </div>
  );
